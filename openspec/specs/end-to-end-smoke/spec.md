# end-to-end-smoke Specification

## Purpose
Unattended verification of the irreducible OS seams that mocks cannot cover:
real `sox` microphone capture, real `osascript` clipboard automation, and a
live `CGEventTap`. All scenarios are tier `macos-real` (convention in
`../TESTING-TIERS.md`), run only under the `macos` marker, and skip cleanly when
the device/permission is absent. The human-in-the-loop flow (hold Fn → speak →
text appears in a focused app) is operator-verified and intentionally not
automated here.
