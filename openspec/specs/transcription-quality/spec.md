...
The system SHALL bias recognition toward a configured custom vocabulary: when an unusual term is supplied via `initial_prompt`, the real model SHALL produce that term in the output. Additionally, the system SHALL accept vocabulary from the correction store via the `hotwords` parameter, providing a second independent biasing channel.
...
#### Scenario: Hotwords term recognized when provided
- **WHEN** a clip naming an unusual term is transcribed with that term supplied as `hotwords`
- **THEN** the cleaned output SHALL contain the term
