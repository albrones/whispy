...
## Purpose
The ports-and-adapters layer that decouples the core engine from any specific
operating system. The OS-coupled seams are expressed as structural port
interfaces (`HotkeyListener`, `TextInjector`, `AudioRecorder`, `TrayUI`, and
`Notifier` `typing.Protocol`s), and a `platform.detect()` factory selects and
binds the adapter set for the current OS at runtime.
